...
fn main() {
    let result: u32 = INPUT.lines().map(calibration).sum();
    println!("part 1 {}", result);
    let result: u32 = INPUT.lines().map(calibration2).sum();
    println!("part 2 {}", result);
}

fn calibration(line: &str) -> u32 {
    let mut digits = line.chars().filter(|c| c.is_numeric()).map(|c | c.to_digit(10).unwrap_or(0));
...
fn calibration2(line: &str) -> u32 {
    let mut digits = line.char_indices().map(|(i, _)| read_digit(&line, i)).filter(|o| o.is_some()).map(|o| o.unwrap_or(0));
    let first = digits.next().unwrap_or(0);
    let last = digits.last().unwrap_or(first);
    first * 10 + last
}

fn read_digit(str: &str, i: usize) -> Option<u32> {
    let chars: &mut Chars = &mut str.chars();
    let c = chars.nth(i)?;
    if c.is_digit(10) {
        c.to_digit(10)
    } else {
        ...
        else if s.starts_with("zero") { Some(0) }
        else { None }
    }
}