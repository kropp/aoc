...
const INPUT: &str = "........................................................862...........20.............453...619......58........694...312.................292.
...
#[derive(Debug)]
struct Part { symbol: char, row: usize, column: usize }

#[derive(Debug)]
struct PartNumber { number: u32, row: usize, start: usize, end: usize }

impl Part {
    fn gear_ratio(&self, numbers: &Vec<PartNumber>) -> u32 {
        if self.symbol == '*' {
            let adjacent: Vec<&PartNumber> = numbers.iter().filter(|n| n.is_adjacent_to(self)).collect();
            if adjacent.len() == 2 {
                adjacent.get(0).unwrap().number * adjacent.get(1).unwrap().number
            } else {
                0
            }
        } else {
...
impl PartNumber {
    fn is_adjacent_to(&self, part: &Part) -> bool {
        if (self.row .. self.row + 3).contains(&(part.row + 1)) {
            let start = if self.start == 0 { 0 } else { self.start - 1 };
            part.column >= start && part.column <= self.end + 1
        } else {
            false
        }
}

    fn is_adjacent(&self, parts: &Vec<Part>) -> bool {
        parts.iter().any(|p| self.is_adjacent_to(p))
    }
}

fn parse(input: &str) -> (Vec<Part>, Vec<PartNumber>) {
    ...
    for line in input.lines() {
        ...
        for (column, c) in line.char_indices() {
            if c.is_digit(10) {
                if column == 0 || !in_number {
                    let end = line[column..].find(|c: char| !c.is_digit(10)).unwrap_or(line.len() - column);
                    let number = line[column .. column + end].parse::<u32>().unwrap();
                    numbers.push(PartNumber { number, row, start: column, end: column + end - 1 })
                }
                in_number = true;
            } else {
                in_number = c.is_digit(10);
                if c != '.' {
                    parts.push(Part { symbol: c, row, column })
                }
            }
        }
        row += 1;
...
fn main() {
    let (parts, numbers) = parse(INPUT);
    let adjacents: u32 = numbers.iter().filter(|n| n.is_adjacent(&parts)).map(|n| n.number).sum();
    println!("{}", adjacents);
    let ratios: u32 = parts.iter().map(|g| g.gear_ratio(&numbers)).sum();
    println!("{}", ratios);
}