...
const _: &str = "Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green";

const INPUT: &str = "Game 1: 2 red, 2 green; 1 red, 1 green, 2 blue; 3 blue, 3 red, 3 green; 1 blue, 3 green, 7 red; 5 red, 3 green, 1 blue
...
impl Game {
    fn is_possible(&self) -> bool {
        self.attempts.iter().all(Attempt::is_possible)
    }
    
    fn power(&self) -> i32 {
        let min_r = self.attempts.iter().map(|a| a.r).max().unwrap();
        let min_g = self.attempts.iter().map(|a| a.g).max().unwrap();
        let min_b = self.attempts.iter().map(|a| a.b).max().unwrap();
        min_r * min_g * min_b
    }
}

fn parse_attempt(line: &str) -> Attempt {
    let mut components: HashMap<&str,i32> = line.split(", ").map(|p| {
        let iter = &mut p.split(" ");
        let count = iter.next().unwrap().parse::<i32>().unwrap_or(0);
        (iter.next().unwrap(), count)
    }).collect();
    Attempt { r: components.remove("red").unwrap_or(0), g: components.remove("green").unwrap_or(0), b: components.remove("blue").unwrap_or(0) }
}

fn parse_game(line: &str) -> Game {
    let mut by_comma = line.split(": ");
    let id = by_comma.next().unwrap()[5..].parse::<i32>().unwrap();
    let sets = by_comma.next().unwrap().split("; ");
    let attempts: Vec<Attempt> = sets.map(parse_attempt).collect();
    Game { id, attempts }
}
...
fn main() {
    ...
    let possible_games = games.clone().filter(Game::is_possible).map(|g| g.id);
    let result: i32 = possible_games.sum();
    println!("{}", result);
    let power: i32 = games.map(|g| g.power()).sum();
    println!("{}", power)
}